...
const app = {
        ...
        <div class="audio-player-container" id="audioContainer-${file.uniqueId}">
            ...
            <div class="audio-section-controls">
                ...
                <div class="audio-btn-row">
                    <button class="icon-btn" onclick="app.toggleLoop('${file.uniqueId}')" id="loopBtn-${file.uniqueId}" title="循环播放" style="color:inherit;opacity:0.8">${svg_loop}</button>
                    <button class="icon-btn" onclick="app.seekAudio(-5, '${file.uniqueId}')" style="color:inherit;font-size:12px;width:36px">-5s</button>
    ...
    updateLoopBtnUI(btn) {
        ...
        if (this.state.loopMode === 'one') {
            btn.style.opacity = '1';
            btn.innerHTML = svg_loop_one; // 使用单曲循环 SVG
            btn.title = "单曲循环";
        } else {
            btn.style.opacity = '0.5';
            btn.innerHTML = svg_loop;     // 使用列表循环 SVG
            btn.title = "无循环";
        }
    },

    seekAudio(seconds, uid) {
