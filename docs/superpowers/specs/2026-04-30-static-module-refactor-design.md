...
6. 新增 `config/config.ts`，专门配置 `defaultPath`。
...
- `config.js`：读取 `config.ts`、合并默认配置、提供分页数量和存储键名等配置。
...
新增 `config/config.ts`，作为用户修改默认路径的唯一文件：

```ts
export const defaultPath = 'D:\\Data\\Share';
```

由于项目不引入构建工具，浏览器不直接执行 TypeScript。`config/js/config.js` 会以文本方式读取 `../config.ts`，解析 `defaultPath` 字符串；读取失败时回退到内置默认路径，并在控制台给出提示。
...
`config.ts` 不能被浏览器当作模块直接执行。缓解方式是将它作为文本配置读取，只解析 `defaultPath` 这一项，并在注释中说明格式要求。
