...
当前项目是无构建步骤的 Everything HTTP Server WebUI，核心文件位于仓库根目录：

- `index.html`：页面结构、SVG 图标和少量内联脚本。
- `index.css`：完整界面样式，约 1874 行。
- `index.js`：完整应用逻辑，约 1998 行。
- `jsmediatags.min.js`：第三方音频标签解析库，约 102 行。
...
使用浏览器原生 ES Module，不需要构建工具。`index.html` 通过 `<script type="module" src="../config/js/app.js"></script>` 启动应用。

`app.js` 负责组合模块并导出全局 `window.app`。这样现有 `onclick="app.xxx()"` 可以继续工作，降低第一次重构的风险。

主要模块职责：

- `config.js`：默认路径、分页数量、存储键名等配置。
...
`index.html` 保留页面骨架和必要的 `onclick` 入口。SVG symbol 继续放在 HTML 中，避免额外请求导致图标闪烁。

内联 JavaScript 中的大段 SVG 字符串会迁移到 `js/icons.js`。样式和脚本引用路径保持兼容 Everything HTTP 的 `../config/` 部署方式。
...
ES Module 的加载路径是主要风险。缓解方式是保留 `../config/` 前缀，并在 `localhost:11080` 上实际加载验证。
